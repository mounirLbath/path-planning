...
In this project, we explore 2 methods for path planning, in a fully known environment: Particle Swarm Optimization and Rapidly-exploring Random Trees. You can find our code here: \href{https://github.com/mounirLbath/path-planning}{Github repository}.\\

Note that in all the provided scenarios, the environment are very simple: there are few obstacles, all of them are rectangles, and we are aware of the full environment at the start. But the methods we will explore can be applied to more complex environments. So we will try our best to keep our implementations as general as possible. For example, we are not basing ourselves on the corner of the rectangles, except when it is required by the questions (intelligent sampling).\\
...
We chose to keep $S=50$ particles, \texttt{max\_iter}$=1000$, \texttt{patience}$=100$, \texttt{nb\_points\_path}$= 10$. We 
...
for k in 1..max_iters:
  for each particle i:
    if random_restart and k % random_period == 0:
        X[i] = RandomPath()
        V[i] = 0
    else:
        classic update
    update P[i] if fitness improves
  update G
\end{verbatim}
...
Let's see how this modifies the pseudo-code: when scanning particles to update the global best, we accept a candidate with the SA probability. If $T_k$ is really small, then $$\min\left(1,\exp\left(-\frac{f(s)-f(g_k)}{T_k}\right)\right)\simeq \mathbf{1}_{f(s) \le f(g_k)}$$
so the behavior is the same as before and we don't get the benefits of simulated annealing. On the other side, if $T_k$ is too big, we will always accept $s$ so the particle are not likely to converge to an optimal solution. 
...
We set $\tau$ (named \texttt{nb\_update\_before\_dim}) around 10--20 iterations.
...
\end{itemize}
For this, we generated particles around a structured configuration (segment or curves $y = x^\epsilon$, like in figure (a)) and added random noise. This keeps a good global structure while still allowing enough randomness for exploration.

Another improvement we thought about, but didn’t implement yet, would be to use a penalty proportional to the distance traveled inside obstacles instead of just counting collisions. This would give a smoother penalty and could help guide the particles more effectively during the search.

In conclusion, Particle Swarm Optimization is very efficient at solving the Path Planning Problem. We will compare it with Rapidly-exploring Random Tree in the next questions.
...
Using this method, we were able to get satisfying results for scenarios 0,1 and 2, but we got stuck again in local minima for 3 and 4. 
...
\href{https://github.com/mounirLbath/path-planning}.
