...
What we do, in a similar fashion as RRT*-Smart \cite{RRT-Smart}, is to, with a certain probability $p_\text{bias}$, sample a point with a different measure than the uniform distribution. In this case, we pick a random interior vertex of the current best path, and sample a point using a truncated gaussian of hyperparameter radius $R_\text{sampling}$ around it. Note that $R_\text{sampling}$ should be lower than $\delta_r$, and it would likely be better to keep it around $\delta_s$ to be consistent with these other hyperparameters.
