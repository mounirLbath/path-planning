...
\begin{enumerate}[nosep]
  ...
  \begin{enumerate}[nosep]
    \item Sample a random free point $v_r$. If a goal path exists, reject samples that cannot improve it (see Q17).
    \item Find the nearest tree node $v_n$ using the grid.
    \item Steer from $v_n$ toward $v_r$ by at most $\delta_s$ to get $v_{\text{new}}$. Discard if the segment hits an obstacle.
    \item Among grid neighbours of $v_{\text{new}}$, pick the collision-free parent to minimize the final cost of reaching $v_{\text{new}}$.
    \item Add $v_{\text{new}}$ to the tree; update parent's children set and grid cell.
    \item Rewire: for neighbours within $\delta_r$, if the path through $v_{\text{new}}$ lowers their cost and is collision free, update their parent, cost, and propagate the cost reduction to all descendants via the children sets.
    \item If $v_{\text{new}}$ sees the goal and the resulting cost improves, either create the goal node or update the existing goal node's parent.
    \item If we just want a path and we found one, stop; otherwise continue.
  \end{enumerate}
\end{enumerate}

\section{Question 16: Complexity of One Iteration}

Let $N$ be the current number of nodes, $O$ the number of obstacles, and $K$ the average number of nodes per grid cell ($K \sim N\delta_r^2 / (x_{\max} y_{\max})$). We detail each step of the pseudocode:
...
\begin{enumerate}[nosep]
  ...
  \begin{enumerate}[nosep]
    ...
    \item \textbf{Steer + collision check.} Steering is $O(1)$ (one direction computation + clamp). The collision check tests the segment $v_n v_{\text{new}}$ against all $O$ obstacles, which is done in $O(1)$ per obstacle, which gives a complexity of $O(O)$.
    ...
    \item \textbf{Termination check.} $O(1)$.
  \end{enumerate}
\end{enumerate}
...
\begin{itemize}
  \item \textbf{Grid buckets for nodes.} Instead of scanning all $N$ nodes for nearest-neighbour and neighbourhood queries, we partition the domain into cells of size $\delta_r \times \delta_r$. This makes the common case $O(K \sim N\delta_r^2 / (x_{\max} y_{\max}))$ per query instead of $O(N)$.
...
% TODO: fill in actual benchmark numbers from RRT_benchmark_results.txt
% For each scenario, report: path length, number of iterations, CPU time.
% Include figures generated by display_environment / display_tree.

Hyperparameter selection: for a $1000 \times 1000$ environment we use $\delta_s \in \{5, 15, 50\}$ and $\delta_r \in \{50, 200\}$. A small $\delta_s$ navigates narrow gaps but grows the tree slowly; a large $\delta_s$ grows fast but may miss tight passages. $\delta_r$ should be several times $\delta_s$ to allow effective rewiring; too large and neighbour scans become expensive, too small and rewiring has little effect.

\begin{center}
\begin{tabular}{c|c|c|c|c|c}
Scenario & $\delta_s$ & $\delta_r$ & Path length & Steps & Time (s) \\ \hline
0 & -- & -- & -- & -- & -- \\
1 & -- & -- & -- & -- & -- \\
2 & -- & -- & -- & -- & -- \\
3 & -- & -- & -- & -- & -- \\
