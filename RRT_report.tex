...
Note that $R_\text{sampling}$ should be much lower than $\delta_r$, and it would likely be better to keep it lower than $\delta_s$ as well to be consistent with these other hyperparameters: $\delta_s$ is here to explore at a reasonable rate, while we are looking for more fine-grained improvement here. We decided here to keep the same parameters as before for $\delta_s$ and $\delta_r$, and set $R_\text{sampling} = 20.0$ and $p_\text{bias} = 0.8$, for $1000$ iterations.

We get a result in 6.18s, with a path length of 1944.26. This is again an improvement over only path optimization for path length, where we had 1968.18 in the previous test. However, it was reached in 462steps and 0.76s. The real reason this is longer is not the sampling time: but since we focus the sampling on certain regions, they become much more charged and it highly increases the complexity of finding the nearest nodes and rewiring. For this, we might want to reduce $\delta_r$ to take advantage of the changes we made. However, for comparison purposes, we are not doing so here. To compare with the actual best we can reach, we ran this same algorithm for 4 000 steps, which took us 97.19s to reach 1941.48 path length, very close to the theoretical best of 1941.32 (computed by hand).
...
\hline
0 & 1446.07/1.02s & FILL & FILL \\
1 & 1457.48/1.80s & FILL & FILL \\
2 & 1514.35/2.45s & FILL & FILL \\
3 & 1540.05/2.44s & FILL & FILL \\
4 & 1987.74/2.11s & FILL & 1944.26/6.18s \\
\hline
\end{tabular}
\end{center}
