...
$c_1=c_2=2.0$, $w=0.8$. Note that having more particles doesn't improve the result here, but it will with the optimizations in the following questions.
...
Random restart periodically reinitializes some (or all) particles.
...
\textbf{Comparison:}
SA tends to:
(i) improve robustness on hard scenarios (avoiding early stagnation),
(ii) add variance in intermediate iterations,
(iii) slightly increase CPU time (extra bookkeeping is negligible; the main cost remains collision checks).
...
Too small: particles become copies of the global best.
Too large: the method rarely activates and has little impact.
