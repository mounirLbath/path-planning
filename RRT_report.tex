...
This gives us, for a path to goal $v_\text{start}, v_1, \ddots, v_P, v_\text{goal}$ a simple $O(P k_\text{rope}\sum_{2\leq i \leq P}D_i)$, which should be very reasonable in practice.

\textbf{Pseudocode (insert at the end of each iteration):}
\begin{lstlisting}
def path_optimization(path = [v_0, v_1, ..., v_P, v_goal], k_rope):
  last_s, i_s = v_P, P-1
  for i = P down to 1:
      for k = 1 to k_rope:
          v_short = ((k_rope - k)/k_rope) * v_i + (k/k_rope) * v_{i-1}
          if line_of_sight(v_goal, v_short):
              insert_in_tree(v_short, parent = v_i-1)
              switch_parent_propagate_costs(v_goal, v_short)
              last_s, is = v_short, i-1
          else:
              path_optimization(path = [v_0, ..., v_is, last_s], k_rope)
              return
\end{lstlisting}
