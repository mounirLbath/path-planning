...
\title{CSC\_42021 Project, PATH PLANNING ALGORITHMS}
\author{Octave GASPARD, Mounir LBATH}


\begin{document}
\maketitle
Code: \href{https://github.com/mounirLbath/path-planning}{Github repository}
...
In this project, we explore 2 methods for path planning, in a fully known environment: Particle Swarm Optimization and Rapidly-exploring Random Trees.

Note that in all the provided scenarios for this project, the environment are very simple: there are few obstacles, all of them are rectangles, and we are aware of the full environment at the start. But the methods we will explore can be applied to more complex environments. So we will try our best to keep our implementations as general as possible. For example, we are not basing ourselves on the corner of the rectangles, except when it is required by the questions (intelligent sampling).\\

In fact, if we consider that our implmentations don't need to work on more complex environments, the problem of path planning can be solved in an extremely simple algorithm. We implemented it in graph\_solve.py. It perfectly solves all the environments we have here, and in general any environment with known polygonal obstacle, in $O(V^3)$, with $V$ is the number of vertices of the obstacles, by running a Dijkstra on the obstacle vertices with a few tricks for collisions. Here, since we have few obstacles, it solves all environments in maximum 0.03s, giving path length of 1443.33 for scenario 0, 1449.06 for scenario 1, 1505.19 for scenario 2, 1522.52 for scenario 3 and 1941.32 for scenario 4, which are the best possible paths.

\section*{Question 1 \& 2: Setting Up the Environment}

We have these functions under environment.py: load\_problem() and display\_environment(). The function makes basic validity check then returns a Problem object, containing all the necessary values :
...
class Problem:
    ...
    start1          # start point of robot 1
    goal1           # goal point of robot 1

    start2          # start point of robot 2
    goal2           # goal point of robot 2

    safety_radius   # for the 2-robots problem
    obstacles       # list of obstacles as a list[Rectangle]
\end{lstlisting}

It contains obstacles as a list of Rectangle objects, all containing methods to help with the implementation of other functions like intersection checks or the graph\_solve.py file.
...
where $N_{\text{intersections}}$ denotes the number of segment–obstacle intersection pairs.
...
\item Parameters:
    \begin{itemize}[nosep]
        ...
        \item \texttt{patience} = time to wait before we early stop the program if no improvement is reported
    \end{itemize}
...
Set $k = 0$ 
\item
While $k<$  max\_iters or global best hasn't changed for \texttt{patience} steps:
...
We implemented a \texttt{Path} class containing:
\begin{itemize}
    \item A set of $n$ intermediate points,
    \item A fixed start point,
    \item A fixed goal point.
\end{itemize}

All particles share the same start and end points. Only the intermediate 
points are optimized during the PSO process.

Initially, paths were generated using identically random uniform variables for the points. This was later improved by sampling points from a Gaussian distribution centered around the straight line between start and goal. To improve this even more, and give a fair repartition of the initial particles in the search space, we also tried a random initialization around curves that fill the screen (Figure~\ref{fig:pso-init}). All our experiments are made with this last initialization as it was the most effective (but the 2 other initializations also work, the algorithm just converges more slowly)

\begin{figure}[!h]
\centering
\begin{subfigure}[t]{0.48\textwidth}
  \includegraphics[width=\textwidth]{pso_figures/Initialization.png}
  \caption{Without noise}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\textwidth}
  \includegraphics[width=\textwidth]{pso_figures/Initialization_plus_random.png}
  \caption{With noise}
\end{subfigure}\hfill

\caption{PSO initialization strategy: initial particles (paths) for scenario 4. (a)~Repartition of paths without random noise. (b)~Paths initialized with same pattern as the left + random noise}
\label{fig:pso-init}
\end{figure}

Initial velocities were set to zero.

To remain consistent with the standard PSO equations, we update the global 
best position only after all particles have been evaluated in a given 
iteration. In other words, the personal bests are updated first for all particles, 
and the global best is computed afterward, rather than updating it 
immediately when a better solution is found during the iteration.

At each iteration, we decided to clamp the norm of the velocities to a certain \texttt{v\_max}. Positions are also clamped to remain within the environment boundaries.

We set a certain \texttt{max\_iters}. We also stop the program as soon as \texttt{best\_fitness} doesn't change for \texttt{patience} steps. 

\section*{Question 8: Results of simple PSO}


Here are the results we obtained. The parameters were chosen thanks to a grid search.

We chose to keep $S=50$ particles, \texttt{max\_iter}$=1000$, \texttt{patience}$=100$, \texttt{nb\_points\_path}$= 10$ and 
$c_1=c_2=2.0$, $w=0.8$. In all figures below, we show all the paths (particles) at the end of the algorithm in blue, and the best path in red.
...
\hline
Scenario & Path length (1000 steps max) & Time & Nb steps (clamped at 1000) \\
\hline
0 & 1443.34 & 3.75s & 1000 \\
1 & 1526.97 & 5.69s & 1000 \\
2 & 1532.07 & 15.50s & 1000 \\
3 & No path & No path & No path \\
4 & No path & No path & No path \\
\hline
\end{tabular}
\caption{Table of results for simple PSO}
\end{figure}

\begin{figure}[!h]
\centering
\begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[width=\textwidth]{pso_figures/scenario0_simple.png}
  \caption{Scenario 0}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[width=\textwidth]{pso_figures/scenario1_simple.png}
  \caption{Scenario 1}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[width=\textwidth]{pso_figures/scenario2_simple.png}
  \caption{Scenario 2}
\end{subfigure}\\[0.5em]
\begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[width=\textwidth]{pso_figures/scenario3_simple.png}
  \caption{Scenario 3}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[width=\textwidth]{pso_figures/scenario4_simple.png}
  \caption{Scenario 4}
\end{subfigure}
\caption{Simple PSO: best path found for each scenario.}
\end{figure}

While choosing the parameters independently for each scenario would have improved the results, we decided not to do so in order to prevent overfitting.

Despite extensive experimentation with different hyperparameter configurations, we were unable to obtain a valid path for Scenario 4 and almost never got a valid path for Scenario 3. In tested cases for these scenarios, the PSO algorithm converged prematurely to a local minimum, producing trajectories that still intersected with an obstacle.

In general, the biggest trade-off is in the choice of S, between speed and quality. This was even more visible for the optimizations in the next few questions.

\section*{Question 9: Random Restart}

Random restart periodically reinitializes some particles.
This helps exploration when the swarm collapses around a poor local optimum or a narrow infeasible region.

Every $P$ iterations (parameter \texttt{random\_period}), we reset:
\[
X_i \leftarrow \text{RandomPath()},\quad V_i \leftarrow 0
\]
for selected particles (in our implementation: for all particles).

\textbf{Tradeoff and parameter choice:}
If \texttt{random\_period} is too small, convergence is constantly disrupted.
If it is too large, restart rarely happens and the benefit disappears.
We used values in the range 10--25 iterations and (after a small grid search) kept it fixed at 15 in our final configuration.

\textbf{Comparison:}
Compared to the basic PSO, random restart enabled us to get a valid solution to scenario 3 by getting out of the local minimum. However, it was still unable to overcome scenario 4.

For Scenarios 0, 1 and 2, the paths obtained with random restarts are slightly longer. This behavior is consistent with the relative simplicity of these environments: since the standard PSO already converges reliably in such cases, introducing random restarts reduces convergence consistency and slightly degrades the quality of the final solution.
...
\hline
\end{tabular}
\caption{Table of results for PSO with random restart}
\end{figure}

\begin{figure}[!h]
\centering
\begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[width=\textwidth]{pso_figures/scenario0_random_restart.png}
  \caption{Scenario 0}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[width=\textwidth]{pso_figures/scenario1_random_restart.png}
  \caption{Scenario 1}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[width=\textwidth]{pso_figures/scenario2_random_restart.png}
  \caption{Scenario 2}
\end{subfigure}\\[0.5em]
\begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[width=\textwidth]{pso_figures/scenario3_random_restart.png}
  \caption{Scenario 3}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[width=\textwidth]{pso_figures/scenario4_random_restart.png}
  \caption{Scenario 4}
\end{subfigure}
\caption{PSO with random restart: best path for each scenario.}
\end{figure}

\section*{Question 10: Simulated Annealing}

Simulated annealing allows occasional acceptance of worse global-best candidates early on, so that the search can escape local minima. Acceptance probability depends on a temperature $T_k$:
$$
\mathbb{P}(\text{accept } s) = \min\left(1,\exp\left(-\frac{f(s)-f(g_k)}{T_k}\right)\right),
\quad
T_k = T_0 \beta^k.
$$

If $T_k$ is really small, then $$\min\left(1,\exp\left(-\frac{f(s)-f(g_k)}{T_k}\right)\right)\simeq \mathbf{1}_{f(s) \le f(g_k)}$$
so the behavior is the same as before and we don't get the benefits of simulated annealing. On the other side, if $T_k$ is too big, we will always accept $s$ so the particles are not likely to converge to an optimal solution. 
The right value for $T_k$ hence depends on $f$. For our fitness function we observe that $\Delta f\sim 10$ for the first few iterations, and $\Delta f\sim 10^4$ when we overcome an obstacle. We hence chose $T = 10000$ and $\beta = 0.95$.

Here is the new pseudo-code for \textfff{update G}
\begin{verbatim}
T = T0
for k in 1..max_iters:
  ...
  for i in 1..S:
    ...
    prob = min(1, exp(-(f(P[i])-f(G))/T)
    Draw u with Uniform(0,1)
...
\textbf{Comparison:}
Despite the promise, simulated annealing didn't make it through scenario 4. It didn't even pass scenario 3, although it did with some fine-tuning for this case, but we wanted the hyperparameters to stay generic. However, we observed simulated annealing to be useful when combined with random restart, and dimensional learning as we will see in the next question. 
...
\hline
\end{tabular}
\caption{Table of results for PSO with random restart and simulated annealing}
\end{figure}

\begin{figure}[!h]
\centering
\begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[width=\textwidth]{pso_figures/scenario0_random_simulated.png}
  \caption{Scenario 0}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[width=\textwidth]{pso_figures/scenario1_random_simulated.png}
  \caption{Scenario 1}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[width=\textwidth]{pso_figures/scenario2_random_simulated.png}
  \caption{Scenario 2}
\end{subfigure}\\[0.5em]
\begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[width=\textwidth]{pso_figures/scenario3_random_simulated.png}
  \caption{Scenario 3}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[width=\textwidth]{pso_figures/scenario4_random_simulated.png}
  \caption{Scenario 4}
\end{subfigure}
\caption{PSO with random restart and simulated annealing: best path for each scenario.}
\end{figure}

\section*{Question 11: Dimensional Learning}

Dimensional learning reduces oscillation by injecting coordinates of the global best into a particle that has stagnated. If particle $i$ has not improved its personal best for $\tau$ steps, we try replacing each waypoint dimension with the global best's corresponding dimension, keeping the change only if fitness improves.

Let \texttt{last\_updated[i]} store the last iteration when $P_i$ was improved. Our new pseudo-code includes

\begin{verbatim}
if k - last_updated[i] >= tau: # k is the current iteration
  for j in 1..n:              # for each dimension
    candidate = X[i]
    candidate[j] = G[j] # update jth coordinate of the path
    if fitness(candidate) < fitness(X[i]):
        X[i] = candidate
\end{verbatim}

\textbf{Hyper-parameters:}
We set $\tau$ (named \texttt{nb\_update\_before\_dim}) around 10--20 iterations (15 for the experiments).
If it's too small, particles become copies of the global best.
If it's too large, the method rarely activates and has little impact.

\paragraph{Comparison:}
With dimensional learning and the previous optimizations, we finally find solutions in all 5 scenarios. 

Dimensional learning often accelerates convergence once a feasible corridor is discovered,
because it helps transfer useful waypoint coordinates without fully collapsing diversity.
It is complementary to random restart and simulated annealing. It enables us to have the consistency and accuracy of simple PSO, with the exploration capacity of random restart and simulated annealing.
Results are remarkably close to the optimal ones found using Dijkstra (cf. introduction)
...
Scenario & Path length (1000 steps max) & Time & Nb steps (clamped at 1000) \\
\hline
0 & 1443.33 & 4.95s & 1000 \\
1 & 1449.15 & 8.97s & 717 \\
2 & 1522.92 & 16.55s & 759 \\
3 & 1522.71 & 28.47s & 1000 \\
4 & 1941.52 & 20.59s & 1000 \\
\hline
\end{tabular}
\caption{Table of results for PSO with random restart, simulated annealing and dimensional learning}
\end{figure}

\begin{figure}[!h]
\centering
\begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[width=\textwidth]{pso_figures/scenario0_all_three.png}
  \caption{Scenario 0}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[width=\textwidth]{pso_figures/scenario1_all_three.png}
  \caption{Scenario 1}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[width=\textwidth]{pso_figures/scenario2_all_three.png}
  \caption{Scenario 2}
\end{subfigure}\\[0.5em]
\begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[width=\textwidth]{pso_figures/scenario3_all_three.png}
  \caption{Scenario 3}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.32\textwidth}
  \includegraphics[width=\textwidth]{pso_figures/scenario4_all_three.png}
  \caption{Scenario 4}
\end{subfigure}
\caption{PSO with random restart, simulated annealing and dimensional learning: best path for each scenario.}
\end{figure}

\section*{Question 12: Other improvements}

To make the algorithm more efficient, we looked at how important the initialization step is. The initial particles need to explore the search space as much as possible in order to:
\begin{itemize}
    \item Avoid getting stuck too early in a local minimum
    \item Reach good solutions faster
\end{itemize}
For this, we generated particles around a structured configuration (segment or curves $y = x^\epsilon$, like in Figure~\ref{fig:pso-init}) and added random noise. This keeps a good global structure while still allowing enough randomness for exploration.

Another improvement we thought about, but didn’t implement yet, would be to use a penalty proportional to the distance traveled inside obstacles instead of just counting collisions. This would give a smoother penalty and could help guide the particles more effectively during the search. We could also think of a penalty that grows over time to enforce that particles try to get out of local minima.

Even without these optimizations, Particle Swarm Optimization is very efficient at solving the Path Planning Problem. We will compare it with Rapidly-exploring Random Tree in the next questions.
...
\section*{Question 19: Path optimization method}
...
\section*{Question 20: Path optimization results}
...
We found a path length 1968.18 for 0.81s of computation, in 460 steps. This is much better than the original 1987.74 in 2.22s we had previously in 1000 steps of optimization. Even though it is not as good as the PSO 1941.52 for 20.59s, it is much faster; and we will see later (Q22) versions of RRT that reach the same level of optimality, but much faster. 
...
\begin{figure}[!ht]
  \centering
  \includegraphics[width=0.5\textwidth]{figures/Path_Optimized_RRT_4_path.png}
...
\section*{Question 21: Intelligent sampling / corner-based methods}
...
\section*{Question 22: Intelligent sampling / corner methods results}

Since we decided not to focus on improvements using assumptions on the environment, we will not discuss much the methods based on sampling corners of obstacles here, and instead will just report the results in the table below, Figure \ref{Q22 corner}. We now discuss the last, "sample around the current best path" method.
...
\begin{subfigure}[t]{0.48\textwidth}
  ...
  \caption{Scenario 4 path with sampling optimization for 1000 steps.}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\textwidth}
  \includegraphics[width=\textwidth]{figures/Sampling_RRT_4_tree_1000.png}
  \caption{Scenario 4 tree with sampling optimization for 1000 steps.}
\end{subfigure}\hfill
\label{fig:rrt-4}
\end{figure}
...
With all these improvements, it is clear that these last versions of RRT are the best to solve the path planning problem from the methods we considered.


This method is clearly the most effective at reaching the optimum in the general case, but it comes with a cost. In Figure \ref{Q22} and \ref{Q22 corner} are the comparisons for all scenarios with exactly the same hyperparameters as what we just did, and optimized over 1000 steps each.
...
\hline
\end{tabular}
\caption{Table of results on same hyperparameters, with $1000$ iterations: $\delta_s = 40.0$, $\delta_r = 150.0$, $k_\text{rope} = 1000$, $p_\text{bias} = 0.8$, $R_\text{sampling} = 20.0$. PL = Path length, PO = Path optimization, SO = Sampling optimization. Notice the interest of this improvement lies mostly in more complex environments like scenario 4 than in very simple ones like scenario 0.}
\label{Q22}
\end{figure}
...
\begin{tabular}{|c|c|c|c|c|}
\hline
Scenario & PL/Time (PO + SC) & PL/Time (PO + RC) & PL/Time (GS)\\
\hline
...
\hline
\end{tabular}
\caption{Table of results with corner-based improvements, with $300$ iterations and same hyperparameters, and $p_\text{sample\_corner} = 0.8$. SC = Sample corners, RC = Rewire corners, GS = Graph solve. Notice all of these methods achieve amazing results since we have simple environments. Sampling the corners doesn't change the general complexity, rewiring the corners introduces a $O(O^2 \frac{\delta_r^2}{x_{\max} y_{\max}})$ term, and the graph solve method is in $O(O^3)$.}
\label{Q22 corner}
\end{figure}

\section*{Question 23}

We propose two ways of dealing with this problem. 

\textbf{First approach:}

We will here detail a theoretical simple solution, which we did not implement, and then explain our way of dealing with the 2 robots in PSO. For the simple solution, we notice a simple fact: if we compute the optimal path for each robot, they intersect at most once (whether on a single point, or on a long path; but when they separate they never intersect again). The proof is easy: if they separate and reunite, one of them will potentially have gone slower than the other, so its path is not optimal, or they will have taken the same time, and they could have taken the same path then. Therefore, we can just compute the best paths individually (for example, with an optimised RRT), then check: if their paths intersect (or get closer than $R$), when both robots arrive at the intersection, we take the robot that has the longest time to reach the goal from this intersection to go first, and the other robot follows him. This minimizes the last-arrival time. In theory, this can make this last-arrival time longer than the optimal 2 robots solution by at most $R$: our method has "maximum path length" (with $L_\text{opt}(ri)$ the optimal path length for robot $i$, $L_{\max} = \max(L_{\text{opt}}(r1), L_{\text{opt}}(r2))$, $L_{\min} = \min(L_{\text{opt}}(r1), L_{\text{opt}}(r2))$) :

$$L_\text{method, last-arrival} \leq \max{(L_{\max}, L_{\min} + R)}$$
The proof is not detailed, but we can notice the "worst-case" is when both robots take the same time to arrive at the intersection; and the rest can be computed. And the optimal solution respects $L_{\text{opt, last-arrival}} \geq L_{\max}$, giving us a solution optimal up to $R$ path length; when we try to minimize the last-arrival time. This method requires to have the optimal best path for both robots; but it would also work if we were just to take our non-optimized path and reconstruct them in case that we have multiple intersections to achieve individual paths with similar properties.

However, we may want to find a better solution, that does not include this $R$ cost:

\textbf{Second :}

For this we use particle swarm optimization again, but this time the particle is defined by the union of the 2 paths of the robots. We update these 2 paths simultaneously. The fitness is now defined as

$$f(\text{path}_1, \text{path}_2) 
= g(\text{length}(\text{path}_1),  \text{length}(\text{path}_2))
+ \lambda \cdot N_{\text{obstacle intersections}} + \lambda'\cdot\mathbf{1}_{\text{robot}_1\text{ and }\text{robot}_2 \text{ collide}}$$

where $g$ is either the $\min$ or the sum of the lengths depending on what we want to achieve (shortest time, or lowest amount of fuel). Checking whether the robots collide is a bit tricky as we want to be able to perform that computation in $\mathcal{O}(n)$ to get the same complexity as before (where $n$ is the number of points on the path). This can be done by using $2$ pointers that you slide on each path and for which you check that they stay at distance at least $2R$ apart. The details are in the code and can be explained during the presentation.

\section*{Question 24}

Using particle swarm optimization, we were able to get satisfying results for scenarios 0,1 and 2, but we got stuck again in local minima for 3 and 4. 

\begin{figure}[!h]
\centering
\begin{subfigure}[t]{0.48\textwidth}
  \includegraphics[width=\textwidth]{robots/robots1.png}
  \caption{2-robots solution for Scenario 1}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\textwidth}
  \includegraphics[width=\textwidth]{robots/robots2.png}
  \caption{2-robots solution for Scenario 2}
\end{subfigure}\hfill
\label{fig:rrt-4}
\end{figure}


Despite that, it is interesting to see that optimal strategies often include curved paths to avoid getting stuck while the other robots passes. You can find videos of the robots moving along our optimal solutions in the Github repository.

\section*{Question 25: AI use}

In this project, for some questions (not all of them! it feels good to type some code the old way sometimes :) ), we used Github Copilot’s tab autocomplete feature to make coding faster.  \\

In addition, for the 2 robots, we asked ChatGPT to write an animated script to visualize the robots and safe zones on the path: it was really helpful to get this vizualisation quickly. We also used AI in the writing of this report to improve the clarity and presentation (for example: AI was useful to format the tables).\\

Finally, we used GitHub extensively for versioning, sharing our code and working in parallel as a team. Our repository is available here: \\ 
\href{https://github.com/mounirLbath/path-planning}{Github Repository Link}.

\section*{Conclusion}

In this project, we investigated two complementary approaches to the path planning problem: Particle Swarm Optimization (PSO) and Rapidly-exploring Random Trees (RRT). While the environment considered was relatively simple, our objective was to design methods that remain as general as possible and highlight the strengths and limitations of each approach.

PSO provides a flexible optimization-based framework by encoding entire paths as particles. It performs well in simple environments but suffers from premature convergence in more constrained scenarios. We showed that incorporating exploration mechanisms such as random restart, simulated annealing, and dimensional learning significantly improves robustness. In particular, dimensional learning proved essential to consistently find feasible solutions in all scenarios while maintaining good path quality.

On the other hand, RRT-based methods offer strong exploration capabilities and quickly find feasible paths, even in complex environments. With appropriate data structures and optimizations such as grid-based nearest-neighbor search, rewiring, and path optimization, RRT achieves a good balance between computational efficiency and solution quality. Moreover, intelligent sampling and corner-based strategies allow the algorithm to approach optimal solutions more systematically.
